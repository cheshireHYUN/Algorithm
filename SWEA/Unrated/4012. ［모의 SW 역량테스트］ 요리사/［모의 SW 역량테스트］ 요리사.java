 ...
 *
 * 시간복잡도
 * N개중 N/2개의 식재료 선택 -> 조합 -> N크기의 boolean[]을 통해 A음식 여부 판별
 * A음식 맛, B음식 맛 차이 구하기 -> 시너지 배열을 모두 방문하여 (i,j)좌표의 식재료가 모두 A인경우 A음식맛에 누적, B면 B에누적
 * 조합은 N C (N/2)이고, N의 최대크기가 16이므로 16C8이고 여기에 곱하기 N^2(시너지 배열탐색) => 16C8 X 16^2
 * 조합에서 30C15가 1억이므로 여유있음...
 */
